...
void	Response::generateResponse()
{
	...
	if (!loc) {
		...
		string uri = _request->getUri() + _server_config.getRoot();
		if (DEBUG) cout << YELLOW << "Using URI: " << uri << RESET << endl;
		string path = root + uri;
		cout << YELLOW << "Constructed path: " << path << RESET << endl;
		if (getPathType(path) == DIRECTORY_PATH) {
			...
			if (getPathType(indexPath) == FILE_PATH) {
				...
				if (DEBUG) cout << GREEN << "Index file exists: " << path << RESET << endl;
				...
				if (DEBUG) cout << RED << "Directory access forbidden (no index, autoindex off1)" << RESET << endl;
				_statusCode = 403;
				_reasonPhrase = generateStatusMessage(_statusCode);
				_responseBody = getErrorPageContent(_statusCode);
				_contentLength = _responseBody.size();
			...
			if (DEBUG) cout << GREEN << "File exists at path: " << path << RESET << endl;
			std::ostringstream	ss;
			std::ifstream		file(path.c_str());
			ss << file.rdbuf();
			_responseBody = ss.str();
