...
void HttpContext::requestParsingStateMachine()
{
	...
	while (can_parse)
	{
		...
		case REQUEST_COMPLETE:
		{
			...
			if (CTX_DEBUG)
				cout << ORANGE << "REQUEST PARSING IS COMPLETE" << RESET << endl;
			can_parse = false;
			break;
		}
		case REQUEST_ERROR:
		{
			if (CTX_DEBUG)
				cout << RED << "REQUEST ERROR" << RESET << endl;
			can_parse = false;
			break;
		}
		default:
...
bool HttpContext::chunkedBodyStateMachine(std::string &buf)
{
	...
	{
		if (buf.size() < _chunkSize)
		{ // Need more data for chunk body
			return false;
		}
