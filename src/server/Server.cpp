...
void Server::addErrorPage(int code, const std::string& page) { _error_pages[code] = page; }
void Server::addLocation(const Location& location) { _locations.push_back(location); }
void Server::setClientMaxBodySize(const std::string& size) { _client_max_body_size = size; }
void Server::addAllowedMethod(const std::string& method) { _allowed_methods.push_back(method); }

size_t Server::getLocationCount() const { return _locations.size(); }
const std::vector<Location>& Server::getLocations() const { return _locations; }
const std::vector<std::string>& Server::getServerNames() const { return _server_names; }
const std::map<int, std::string>& Server::getErrorPages() const { return _error_pages; }
const std::string& Server::getClientMaxBodySize() const { return _client_max_body_size; }
const std::vector<std::string>& Server::getAllowedMethods() const { return _allowed_methods; }
